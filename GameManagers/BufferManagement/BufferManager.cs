...
namespace GameManagers.BufferManagement
{
    public class BufferManager : IInitializable, IBufferManager
    {
        ...
        public BufferManager(
            ...
            IResourcesServices resourcesServices, RelayManager relayManager)
        {
            ...
            string buffIconPath)
        {
            foreach (Collider playersCollider in targets)
            {
                if (playersCollider.TryGetComponent(out NetworkObject playerNgo) == false)
                {
                    playerNgo = playersCollider.GetComponentInParent<NetworkObject>();
                }
