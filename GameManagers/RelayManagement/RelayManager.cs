...
namespace GameManagers.RelayManagement
{
    public enum RelayDisconnectCause
    {
        Unknown,
        IntentionalLeaveToLobby,
        ApplicationQuit,
        MigrationRecovery
    }
    ...
    public class RelayManager : IInitializable
    {
        ...
        public  RelayDisconnectCause DisconnectCause { get; private set; } 
        //플레이씬에서 종료할때의 프로퍼티 true로 바꿔야 로비 이전할때의 동작이 수행 안되고 바로 종료됨
        ...
        {
            if (DisconnectCause != RelayDisconnectCause.ApplicationQuit) // 로비씬 이동
            {
                _signalBus.Fire(new RelayDisconnectedSignal()
                {
        ...
        {
            DisconnectCause = RelayDisconnectCause.Unknown; 
            ...
            ResitedClientDisConnectCallBack();
        ...
        public async UniTask<bool> JoinGuestRelay(string joinCode)
        {
            ...
            try
            {
                ...
                UtilDebug.Log($"조인코드{_joinCode}");
                
                bool isSuccess = !string.IsNullOrEmpty(joinCode) && NetworkManagerEx.StartClient();
                if (isSuccess)
                {
                    // 접속 성공 시 콜백 구독
                    ResitedClientDisConnectCallBack();
        ...
        /// <summary>
        /// 2.27일 릴레이 서버 연결이 끊어 졌을때의 콜백함수
        /// 주로 클라나 호스트의 연결이 끊어질때 이용
        /// </summary>
        private void ResitedClientDisConnectCallBack()
        {
        ...
        public void ShutDownRelay(RelayDisconnectCause cause)
        {
            DisconnectCause = cause;
            _choicePlayerCharactersDict.Clear();
            _ngoRPCCaller = null;
            if (NetworkManagerEx != null && (NetworkManagerEx.IsListening || NetworkManagerEx.ShutdownInProgress))
            {
                NetworkManagerEx.Shutdown();
            }
            _joinCode = null;
        }
        public async UniTask WaitForRelayShutdownAsync()
        {
            await UniTask.WaitUntil(() =>
                NetworkManagerEx != null &&
                !NetworkManagerEx.IsListening &&
                !NetworkManagerEx.ShutdownInProgress);
        }
        ...
        {
            _socketEventManager.DisconnectRelayEvent -= ShutDownRelayWrapper;
            _socketEventManager.DisconnectRelayEvent += ShutDownRelayWrapper;
        }

        private UniTask ShutDownRelayWrapper(RelayDisconnectCause cause)
        {
            ShutDownRelay(cause);
