...
namespace GameManagers.VFXManagement
{
    ...
    public class VFXManager : IVFXManagerServices
    {
        ...
        /// <summary>
        /// 4.1일 추가.
        /// 플레이어가 전멸을 하면, 릴레이서버를 끊어버리는데,
        /// 이전에 요청했던 RPC건 때문에 Null이 뜨는 문제가 발생하여 방어코드를 넣음 
        /// </summary>
        /// <returns></returns>
        private bool CanSpawnNetworkVfx()
        {
            if (_relayManager.NetworkManagerEx == null)
            {
                return false;
            }
            ...
            if (_relayManager.NetworkManagerEx.ShutdownInProgress)
            {
                return false;
            }
        ...
        {
            FollowingGenerator(chaseTr, particleObject).Forget();
        ...
        public async UniTaskVoid FollowingGenerator(Transform targetTr, GameObject particle)
        {
            if (particle == null)
            {
                return;
            ...
            if (targetTr == null)
            {
                _resourcesServices.DestroyObject(particle);
                return;
            }
            ...
            CancellationTokenSource linkedTokenSource = CancellationTokenSource.CreateLinkedTokenSource(
                ...
                targetTr.GetCancellationTokenOnDestroy());

            try
            {
                while (particle != null && particle.activeSelf == true && targetTr != null)
                {
                    particle.transform.position = new Vector3(targetTr.position.x, particle.transform.position.y,
                        targetTr.position.z);

                    await UniTask.NextFrame(linkedTokenSource.Token);
                }
            }
            ...
            if (particle != null && targetTr == null)
            {
                _resourcesServices.DestroyObject(particle);
            }
        }
