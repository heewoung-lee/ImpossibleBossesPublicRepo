...
{
    public class MockUnitNetworkFirstBossScene : ISceneSpawnBehaviour
    {
        ...
        [Inject]
        public MockUnitNetworkFirstBossScene(
        ...
        private NgoFirstBossSceneSpawn _ngoGamePlaySceneSpawn;
        ...
        {
            if (clientID != _relayManager.NetworkManagerEx.LocalClientId) //남이 접속할때는 무시하고, 내가 접속할때만 등록
                return;
            ...
            void InitNgoPlaySceneOnHost()
            {
                if (_relayManager.NetworkManagerEx.IsHost)
                {
                    _ngoGamePlaySceneSpawn = _resourceService.InstantiateByKey("Prefabs/NGO/NgoBattleSceneSpawn")
                        .GetComponent<NgoFirstBossSceneSpawn>();
