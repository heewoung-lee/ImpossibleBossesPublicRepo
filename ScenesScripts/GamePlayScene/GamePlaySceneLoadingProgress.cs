...
namespace ScenesScripts.GamePlayScene
{
    public class GamePlaySceneLoadingProgress : UIBase
    {
        ...
        private bool _hasStartedLoadingFadeOut = false;
        private Coroutine _loadingProgressCoroutine;
        private Action _onLoadingFadeOutStart;
        private Action _onLoadingComplete;

        public event Action OnLoadingFadeOutStart
        {
            add
            {
                UniqueEventRegister.AddSingleEvent(ref _onLoadingFadeOutStart, value);
            }
            remove
            {
                UniqueEventRegister.RemovedEvent(ref _onLoadingFadeOutStart, value);
            }
        }
        ...
        private void EnsureTotalPlayerCount()
        {
            if (_totalPlayerCount > 0)
            {
                return;
            }
        ...
        private IEnumerator LoadingSceneProcess(int playerCount)
        {
            ...
            while (_uiLoading.LoaingSliderValue <= 1f)
            {
                ...
                {
                    if (_hasStartedLoadingFadeOut == false)
                    {
                        _hasStartedLoadingFadeOut = true;
                        _onLoadingFadeOutStart?.Invoke();
                    }
        ...
        private IEnumerator FadeOutLoadedScene(Image[] loadSceneImages)
        {
            ...
            _hasStartedLoadingFadeOut = false;
